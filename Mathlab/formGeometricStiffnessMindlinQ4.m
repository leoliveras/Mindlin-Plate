...
for iElement=1:N_elements       
    ...
    for n=1:size(gaussWeights_reduced,1)                      
        ...
        [~,N_diff_xi_eta_rows]=shapeFunctionQ4(xi_reduced_Gauss,eta_reduced_Gauss);
        [J_mat,N_diff_x_y_rows]=Jacobian(nodeCoordinates(i_nodes,:),N_diff_xi_eta_rows);

        % geometric matrix
        G_b=zeros(2,3*N_nodesPerElement);
        G_b(1,1:N_nodesPerElement)=N_diff_x_y_rows(1,:);
        G_b(2,1:N_nodesPerElement)=N_diff_x_y_rows(2,:);
...
for n=1:size(gaussWeights_reduced,1)                      
    ...
    G_s1(1,N_nodesPerElement+1:2*N_nodesPerElement)=N_diff_x_y_rows(1,:);
    G_s1(2,N_nodesPerElement+1:2*N_nodesPerElement)=N_diff_x_y_rows(2,:);
    ...
    G_s2(1,2*N_nodesPerElement+1:3*N_nodesPerElement)=N_diff_x_y_rows(1,:);
    G_s2(2,2*N_nodesPerElement+1:3*N_nodesPerElement)=N_diff_x_y_rows(2,:);
